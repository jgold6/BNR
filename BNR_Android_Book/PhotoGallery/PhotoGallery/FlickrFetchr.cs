...
//				Console.WriteLine("[{0}] Fetchitems Start GetUrlAsync: {1}", TAG, DateTime.Now.ToLongTimeString());
				string xmlString = await GetUrlAsync(url).ConfigureAwait(false);
//				Console.WriteLine("[{0}] Fetchitems End GetUrlAsync: {1}", TAG, DateTime.Now.ToLongTimeString());
